...
class VideoSearchSystem:
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        if config_path:
            ...
            default_config_path = Path(
                __file__).parent / 'config' / 'default.yaml'
    ...
    def _initialize_components(self):
        ...
        self.index = OptimizedHNSWIndex(
            ...
            level_generation_factor=index_config.get(
                'level_generation_factor', 1.0 / np.log(2.0)),
    ...
    async def add_video(self, video_path: str, video_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            features_data = self.feature_extractor.extract_from_video_frames(
                frames_data)
            ...
            self.metrics.record_histogram(
                'index_insertion_time', index_time * 1000)
            ...
            self.metrics.record_histogram(
                'total_ingestion_time', total_time * 1000)
    ...
    async def search(
        ...
        try:
            ...
            if isinstance(query, str):
                # Text query
                query_vector = self.feature_extractor.extract_text_features(
                    query)
            else:
                # Image query - assume it's already a numpy array or process it
                if not isinstance(query, np.ndarray):
                    query_vector = self.feature_extractor.extract_features(
                        query)
                else:
                    query_vector = query
            ...
            self.metrics.record_histogram(
                'query_feature_extraction', feature_time * 1000)

            # Step 3: Search HNSW index
            search_start = time.time()
            search_results = self.index.search(
                query_vector, k * 2)  # Over-fetch for deduplication

            index_search_time = time.time() - search_start
            self.metrics.record_histogram(
                'index_search_time', index_search_time * 1000)
            ...
            self.metrics.record_histogram(
                'result_enrichment_time', enrichment_time * 1000)

            # Step 5: Cache results
            if use_cache and final_results:
                if isinstance(query, str):
                    self.query_cache.cache_results(
                        None, k, final_results, query_text=query)
                else:
                    self.query_cache.cache_results(
                        query_vector, k, final_results)
    ...
    def list_videos(self, limit: int = 100, offset: int = 0) -> List[Dict[str, Any]]:
        ...
        """
        video_keys = [k for k in self.video_metadata.keys()
    ...
    def delete_video(self, video_id: str) -> bool:
        ...
        try:
            ...
            logger.warning(
                "Video deletion requires index rebuild (not implemented)")
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            if self.index.size() > 0:
                # Test search
                test_vector = np.random.randn(
                    self.config['index']['dimension'])
